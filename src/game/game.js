...
export default class Game {
   ...
   * @param {Object} battleData - Optional pre-loaded data from Supabase
   *   @param {Object} battleData.stage - Stage configuration
   *   @param {Array} battleData.heroes - Pre-loaded hero instances
   *   @param {Array} battleData.enemies - Pre-loaded enemy instances
   */
  constructor(canvas, ctx, battleData = null) {
    this.canvas = canvas;
    this.ctx = ctx;
    this.lastTime = 0;
    this.camera = { x: 0, y: 0 };
    ...
    if (battleData) {
      console.log('[Game] Using pre-loaded Supabase data');
      this.stageData = battleData.stage;
      this.preloadedHeroes = battleData.heroes;
      this.preloadedEnemies = battleData.enemies;
    } else {
      console.log('[Game] Using local JSON fallback');
      this.stageData = loadStageData();
      this.preloadedHeroes = null;
    ...
    // Use CSS/logical width for grid calculations (not internal canvas.width which may be scaled for HiDPI)
    const logicalWidth = canvas.clientWidth || canvas.width;
    this.grid = new Grid(6, 12, logicalWidth);
  ...
  /**
   * Initialize units - uses preloaded data or falls back to async loading
   */
  _initializeUnits() {
    if (this.preloadedHeroes && this.preloadedHeroes.length > 0) {
      // Use preloaded heroes from Supabase
      console.log('[Game] Using preloaded heroes:', this.preloadedHeroes.length);
      this.battle.heroes = this.preloadedHeroes;
    ...
    if (this.preloadedEnemies && this.preloadedEnemies.length > 0) {
      // Use preloaded enemies from Supabase
      console.log('[Game] Using preloaded enemies:', this.preloadedEnemies.length);
      this.battle.enemies = this.preloadedEnemies;
      // Initialize pixel positions
  ...
  render(ctx) {
    // Use logical (CSS) dimensions for rendering calculations
