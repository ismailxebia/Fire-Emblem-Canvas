...
export function handleInput(game) {
  ...
  // Helper: cari hero/unit di cell tertentu
  ...
  let startY = 0;
  let hasMoved = false;
  const dragThreshold = 10; // threshold yang sedikit lebih tinggi agar tidak salah deteksi tap
  ...
  game.canvas.addEventListener('mouseup', (e) => {
    if (!hasMoved) {
      // Ini dianggap sebagai tap/click
      ...
      // Proses seleksi hero dan action menu (kode sesuai logika asli)
  ...
  // Event untuk sentuhan (mobile) menggunakan drag yang sama
  ...
  game.canvas.addEventListener('touchmove', (e) => {
    ...
    e.preventDefault(); // Pastikan untuk mencegah scroll native
  }, { passive: false });
  ...
  // Optional: keyboard events untuk debugging/desktop
