...
export default class Battle {
    ...
    render(ctx, camera) {
        ...
        if (this.actionMode === 'move' && this.pendingMove && this.pendingMove.newPosition) {
            ...
            if (path.length > 0) {
                ctx.save();
                // Buat helper function untuk mendapatkan center cell
                ...
                // Buat gradient dari titik awal hingga titik akhir path
                ...
                gradient.addColorStop(0, 'rgba(138, 127, 255, 0.8)');  // Mulai dengan orange
                gradient.addColorStop(1, 'rgba(144, 89, 255, 0.8)');      // Akhiri dengan merah

                ctx.strokeStyle = gradient;
                ctx.lineWidth = 20;
                ctx.lineJoin = 'round'; // Pastikan sudut yang membulat
                ctx.lineCap = 'round';  // Pastikan ujung garis membulat

                ctx.beginPath();
                // Mulai dari titik tengah cell pertama
                let prev = getCellCenter(path[0]);
                ctx.moveTo(prev.x - camera.x, prev.y - camera.y);

                // Untuk setiap node di path, gunakan arcTo untuk membuat sudut rounded
                for (let i = 1; i < path.length; i++) {
                    const current = getCellCenter(path[i]);
                    // Jika bukan node terakhir, gunakan arcTo dengan target node berikutnya
                    if (i < path.length - 1) {
                        const next = getCellCenter(path[i + 1]);
                        ctx.arcTo(current.x - camera.x, current.y - camera.y,
                            next.x - camera.x, next.y - camera.y,
                            12); // Corner radius 12px
                    } else {
                        // Jika node terakhir, langsung lineTo
